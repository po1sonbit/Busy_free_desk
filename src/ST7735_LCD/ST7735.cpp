...
void st7735_init() {
  ...
  buscfg.quadwp_io_num = -1,  // unused
  buscfg.quadhd_io_num = -1,  // unused
  buscfg.max_transfer_sz = LCD_WIDTH * LCD_HEIGHT * 2;
  buscfg.flags            = SPICOMMON_BUSFLAG_MASTER;
  buscfg.intr_flags       = ESP_INTR_FLAG_LOWMED;

  spi_device_interface_config_t devcfg;
  devcfg.clock_speed_hz = 10 * 1000 * 1000,   // Clock out at 10 MHz
  devcfg.mode = 0,                            // SPI mode 0
  devcfg.spics_io_num = PIN_NUM_CS,           // CS pin
  devcfg.queue_size = 7,                      // Queue 7 transactions at a time
  devcfg.pre_cb = lcd_spi_pre_transfer_callback; // Pre-transfer callback
  #endif

  spi_host_device_t spi_host = SPI2_HOST;
#ifdef CONFIG_ST7735_HOST_HSPI
  spi_host = HSPI_HOST;
#elif CONFIG_ST7735_HOST_VSPI
  spi_host = VSPI_HOST;
#endif
